...
class Query {
  constructor(options = {}) {
    ...
    // Extend the timeout from 10 seconds to 30 seconds
    this.timeout = timeout || 30000 // 30 seconds
    this.actions = []
    this._context = {}
  }
  ...
  go(url) {
    ...
    this.actions.push(async ({ page, results } = {}) => {
      ...
      // Add the waitUntil value to prevent timeouts
      let resp = await page.go(url, { timeout, waitUntil: 'networkidle2' })
